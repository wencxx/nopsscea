        ...
        <div v-for="(sport, index) in sports.filter(s => filteredSchedule(s).length)" :key="index" class="space-y-2">
            <div class="space-y-2">
                ...
                <div class="grid lg:grid-cols-2 gap-4">
                    <div v-for="schedule in filteredSchedule(sport)" :key="schedule.id" :class="{ '!bg-blue-900': index % 2 === 0 }" class="bg-red-600 h-24 lg:h-16 py-9 px-5 rounded-lg flex justify-between items-center cursor-pointer" @click="openScoreboard(schedule)">
